...
void main(void)
{
    ...
    mtx_init_dp(&myTestMatx,&TestMatrix_1_2x3[0][0],NROWS(TestMatrix_1_2x3),NCOL(TestMatrix_1_2x3));
    //show_matrix(&TestMatrix_1_2x3[0][0],2,3);
    //show_matrix_obj(myTestMatx);


    (void)mtx_init_dp(&myFactMatrix,&symMtx[0][0],NROWS(symMtx),NCOL(symMtx));
    show_matrix(&symMtx[0][0],5,5);

    (void)mtx_chol_dp(myFactMatrix);
